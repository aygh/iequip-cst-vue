...
request.create = function (protoName, obj) {
  ...
  // console.log($myDecode(req, 'com.ccb.cloud.protobuf.proto.iequip.AcctRegister'));
...
export const $axios = function (config) {
  let req = null;
  if (config['data'] && JSON.stringify(config['data']) !== '{}' && config['PBreq']) {
    req = request.create(config['PBreq'], config['data']); //对请求参数做处理，得到一个 arrayBuffer
  }
  
  return request(config['method'].toLowerCase(), config['url'], req, config['PBres'])
}
